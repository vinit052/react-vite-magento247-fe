...
function Pagination({ totalPagas }: Props) {
    ...
    for (let i = 1; i <= totalPagas; i++) {
        if (i == 5) { break; }
        const style = i === filter.currentPage ? 'p-2 rounded-full bg-blue-500 text-white hover:bg-blue-600' : 'p-2 rounded-full bg-gray-200 text-gray-600 hover:bg-gray-300';
        pageSelectors.push(<button key={i} value={i} onClick={handlePageNumber} className={style}>{i}</button>)
    }
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div>
                        ...
                        <select value={filter.pageSize} onChange={handlePageSizeChange} id="pageSize" name="pageSize" className="ml-2 p-2 rounded border border-gray-300">
                    ...
                    <div className="text-sm text-gray-600">
                        <span>Page <span id="currentPage">{filter.currentPage}</span> of <span id="totalPages">{totalPagas}</span></span>
                    </div>
                </div>

                {(totalPagas > 1) && pageButtons(pageSelectors)}
